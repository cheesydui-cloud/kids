        ...
        <div className="p-5">
          {rule.entry ? (
            <div className="space-y-2">
              <div className="flex items-center gap-2.5 bg-[#0e1117] rounded-lg px-4 py-3">
                ...
                {rule.entry_node_id && node_by_id[rule.entry_node_id] && <span className="bg-transparent text-indigo-200 border-[1.5px] border-indigo-400/45 text-[11px] font-semibold px-2 py-0.5 rounded">{node_by_id[rule.entry_node_id].name}</span>}
          ...
          <div className="grid grid-cols-[90px_1fr] gap-4 items-center mt-5 text-sm">
            ...
            <span className="inline-flex items-center gap-1.5 font-mono">
              {node ? <Link to={`/nodes/${node.id}`} className="link-accent hover:underline">{node.name}</Link> : `#${rule.node_id}`}
            </span>
              ...
              {hops.map(h => {
                ...
                return (
                  <tr key={h.position}>
                    ...
                    <td className="font-semibold"><Link to={`/nodes/${h.node_id}`} className="link-accent hover:underline">{hopNode?.name || `#${h.node_id}`}</Link></td>
