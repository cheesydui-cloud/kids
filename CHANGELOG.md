...
落地仓库编辑节点滚轮拉不到备注、到期时间、保存：卡片 `overflow:hidden` 把滚轮吃掉，遮罩又禁止滚动。

### 修复

- 长弹窗改成整层遮罩跟鼠标滚轮走，卡片不再限高裁内容，能滚到备注、到期时间、保存
- 公告发布/编辑、登录公告同样改

### 升级注意

- 仅面板前端改动，不改数据库、节点、规则、用户、流量或订阅口令
- 节点 agent 无需升级；面板升级会短暂重启，节点会在恢复后自动重连
- 升级后侧栏应显示 **v0.7.10**。若仍裁切，硬刷新一次（Cloudflare 可能还在送旧 JS）

---

## v0.7.9 — 2026-09-19

落地仓库编辑节点「管理」下面被圆角裁掉、没有滚动条：`max-height: 100%` 在 flex 子项上不生效。

### 修复

- 弹窗改为 `max-height: calc(100dvh - 3rem)`，表单区强制 `overflow-y: auto`，能滚到备注、到期时间、保存
- 公告发布/编辑、登录公告同样改
...
落地仓库编辑节点仍滚不到到期时间：弹窗作为 flex 子项时 `min-height:auto` 按内容撑开，内部滚动从未生效。

### 修复

- 公共弹窗改为遮罩 flex 居中，面板 `min-h-0 max-h-full`，标题固定，表单在卡片内滚动到保存
- 公告发布/编辑、登录公告同样改
...
落地仓库「添加节点」弹窗限高后仍滚不到保存：改成整层遮罩滚动。

### 修复

- 弹窗不再自己限高。限高后遮罩内容刚好等于窗口，滚条出不来，底部保存仍然够不着
- 公告发布/编辑、登录公告同样走遮罩滚动
...
修复落地仓库「添加节点」等长弹窗滚不到保存按钮。

### 修复

- 公共弹窗内容高于窗口时，不再被居中裁掉顶部和底部；标题固定，中间可滚到保存
- 公告发布/编辑、登录公告长文同样能滚到底部按钮
...
国内机器安装节点时，勾选后直接复制可用命令：从 GitHub 镜像拉 nft-agent，不再要求本机 7890 代理。

### 新增 / 改进

- 节点详情「节点安装命令」勾选「国内机器」后，命令改为 `gh-proxy.com` 拉取 `install.sh` 和二进制，装好后仍用 token 直连面板
- 不再默认填 `http://127.0.0.1:7890`。那台机器上没有 clash 时，curl 会立刻失败

### 升级注意

- 先升级面板，再重新复制安装命令
- 节点 agent 无需仅为这次升级；已装好的节点控制信道不变
- 面板升级会短暂重启，节点会在恢复后自动重连

---

## v0.7.4 — 2026-09-14

节点安装命令可勾选走代理，方便国内机器从国外面板拉取安装脚本和 nft-agent。

### 新增

- 节点详情「节点安装命令」增加「走代理安装」：勾选后填写 HTTP/SOCKS 代理（默认 `http://127.0.0.1:7890`），复制出的命令会带外层 `curl -x` 和脚本 `--proxy`
- `/v1/install-agent` 识别 `--proxy`，下载 nft-agent 与后续 `nft-upgrade` 都走同一条代理；装好后的控制信道仍直连面板

### 升级注意

- 先升级面板，再重新复制节点安装命令；旧面板没有勾选，手工加 `curl -x` 也拉不到带 `--proxy` 的安装脚本
- 节点 agent 无需仅为这次升级；已装好的节点控制信道不变
- 面板升级会短暂重启，节点会在恢复后自动重连
...
修复从线路监控点进落地仓库（以及其它列表页互相切换）时白屏：`TypeError: R is not a function`。

### 修复

- 线路监控、落地仓库、用户、规则、公告等页面不再把返回 Promise 的 `load` 直接交给 `useEffect`。离开页面时 React 会把这个 Promise 当成卸载清理函数调用，压缩后就是「R is not a function」
- 同一写法在节点详情、用户详情、规则详情、从落地仓库导入弹窗里一并改掉，避免同类白屏
...
修复线路监控按「速度」排序会白屏，以及分组人数把隐藏的自身节点算进去的问题。

### 修复

- 线路监控按「速度」排序时，不再在 `setSort` 的 updater 里改快照状态，避免 React 在渲染阶段二次 setState 导致页面崩溃
- 节点分组人数和「未分组」计数不再包含隐藏的自身节点，避免列表少一台、未分组却多 1

### 升级注意

- 仅面板改动，不改节点、规则、用户、流量或订阅口令
- 节点 agent 无需升级；面板升级会短暂重启，节点会在恢复后自动重连
...
修复线路监控在数据加载前后因 Hook 顺序变化可能崩溃的问题。

### 修复

- 线路监控把速度排序快照的 `useEffect` 挪到加载/错误提前返回之前，避免首次无数据再有数据时 Hook 顺序不一致
- 去掉加载完成后重复注册的同一 effect，只保留一份
...
本版本把「线路监控」从“能看”升级成“能管”：节点可以分组，能记录备注、到期时间和月成本，异常节点一键筛出，实时速度改为逐行刷新。

### 新增

- 节点分组：线路监控可按分组筛选，支持新建 / 重命名 / 删除分组；删除分组只把节点移回「未分组」，不会删除节点
- 节点运维信息：可为每台节点记录分组、备注、到期时间与月成本，列表新增「到期 / 成本」列并在临近到期时显示提醒徽章
- 线路监控新增状态筛选（需处理 / 错误 / 警告 / 离线 / 待同步 / 已禁用 / 正常）：有异常时列表顶部提示「N 个节点需要处理」，点一下即可只看这些节点

### 修复

- 只作为组合链路成员、计费量为 0 的中转节点不再显示成「0 B」，改为「—」并说明计费流量记在组合节点上
- 搜索节点不再只匹配名称：现在支持名称、连接 IP、中继域名、备注、分组名和节点编号

### 改进

- 节点列表的实时速度改为逐行订阅：只有数值变化的那一行重新渲染，不再每秒重绘整张表；只有按「速度」排序时才临时订阅全量数据
- 备注、到期时间、月成本等运维字段只对管理员可见，普通用户接口不会返回这些信息

### 升级注意

- 面板升级会自动执行迁移 0066（为节点增加分组、备注、到期、成本字段，并新建 node_folders 表），不改动现有节点、规则、用户、流量或订阅口令
- 本次是面板改动，节点 agent 无需升级；面板升级会短暂重启，节点会在恢复后自动重连
- 升级后在「线路监控」即可看到新的分组筛选、状态筛选和「到期 / 成本」列

---

## v0.6.0 — 2026-09-10

本版本集中改进面板的移动端操作、键盘无障碍与运行可观测性：手机端管理页可以直接新建，租户可自行切换深浅色，公告已读跟随账号同步，并新增只读的审计日志页。

### 新增

- 新增「审计日志」页（系统 → 审计日志）：按时间倒序查看关键操作，支持关键字、时间范围筛选与分页，只读、仅管理员可见
- 公告已读状态跟随账号同步（新增 announcement_reads 表）：换浏览器或换设备不再重新变成未读，登录弹窗关闭或超时也会记已读
- 落地仓库支持批量删除：一次请求删除多个节点，并在审计日志记录 node_repo.batch_delete
- 普通用户门户新增深色/浅色切换，与管理员顶栏行为一致，未选择时继续跟随系统

### 修复

- 手机端管理列表（用户管理 / 转发规则 / 线路监控）不再隐藏「新建用户 / 创建规则 / 添加节点」主按钮
- 页面加载失败不再伪装成「暂无数据」或「不存在」：落地仓库、节点/用户/规则详情、落地导入选择器都会给出错误提示和重试入口
- 移动端规则卡片上点击测速或二维码不再误跳转到规则详情
- 弹窗支持 Esc 关闭、Tab 焦点不再跑到背景、关闭后焦点回到触发按钮，打开期间背景滚动被锁定

### 改进

- 转发规则的「网速」列改为逐行订阅：只有数值变化的那一行重新渲染，规则数量较大时不再每秒重绘整张表
- 表格排序表头改为真正的按钮并带 aria-sort；下拉筛选、行点击、一键复制、公告已读等支持键盘与读屏操作
- 测速、更多操作、重新同步、删除节点等图标按钮补充无障碍名称

### 升级注意

- 面板升级会自动执行迁移 0065 创建 announcement_reads 表，不改动节点、用户、授权、规则、流量或订阅口令；本次是面板改动，节点无需升级
- 「审计日志」展示升级后新产生的记录；审计记录仍按原有保留策略清理（默认保留 180 天）
- 手机端管理页只保留主按钮，粘贴授权、同步所有、一键升级、组合节点等次要操作在桌面宽度显示

---

## v0.5.0 — 2026-09-09

本版本集中优化大规模规则列表、统计可靠性、历史数据维护和发布验证，不改变已有节点、用户、授权、规则、流量或订阅口令。

### 新增

- 审计日志、小时流量和日流量增加可控保留策略，默认分别保留 180 天、30 天和 730 天
- 数据库增加审计、规则路径和历史流量索引，降低管理列表和清理任务的查询成本
- 本地数据库备份完成后自动执行 SQLite 完整性检查，坏快照不会被保留
- Release 额外发布 `SHA256SUMS.sig` 和 `SHA256SUMS.pem`，支持用 Cosign/Sigstore 校验发布身份

### 修复

- 用户详情、订阅、个人规则和规则详情接口批量加载规则路径，避免规则数量增加后出现 N+1 查询
- 仪表盘、节点列表和相关详情接口不再把统计数据库错误静默显示为 0 或空数据
- 审计写入失败会记录明确错误，便于发现数据库只读、磁盘满等运维问题

### 改进

- 前端管理员页面按路由懒加载，用户首屏不再下载全部管理页面代码
- 历史清理只触及审计和统计历史表，不触及节点、用户、授权、规则和累计流量

### 升级注意

- 普通升级只执行增量迁移；新增索引不会删除或重建现有业务数据
- 默认会清理超过保留期的审计与历史统计记录；如需长期保留，可将对应 `*-retention-days` 参数设为 `0`
- 面板升级会短暂重启，节点 agent 不会被强制替换，现有节点连接会在面板恢复后自动重连

---

## v0.4.9 — 2026-09-09

本版本集中修复面板安全边界、节点创建一致性和异常处理问题，不改变已有节点、用户、规则和流量数据。

### 修复

- 登录页不再把密码保存到浏览器 `localStorage`，旧格式会自动清理，仅保留用户名
- 临时网络错误或 5xx 不再误判为退出登录
- 安装脚本只接受经过校验的面板来源，反向代理头仅信任受信代理，避免 shell 注入
- 订阅上游非 2xx 响应和订阅数据库读取错误不再被当成空订阅成功返回
- 节点创建改为事务，非法端口、非法组合模式、无效授权不会留下半成品节点
- 组合节点拒绝重复子节点和嵌套组合，避免规则展开失败
- 规则下发关键查询失败会明确报错，不再把错误数据当成空规则下发

### 改进

- 规则列表批量加载规则跳转，减少大量规则时的数据库 N+1 查询
- 面板升级不再执行 GitHub `main` 分支的未固定脚本，只使用本机已安装并通过校验的升级器

### 升级注意

- 普通面板升级只执行数据库迁移，不覆盖现有 `panel.db`；节点用户、授权、规则、流量和订阅口令会保留
- 面板升级后远程 agent 不会被强制替换，按节点升级状态单独推送即可

---

## v0.4.8 — 2026-09-07

管理员可以把整台面板无损搬到另一台机器：一次导出全部数据，导入整盘覆盖。

### 新增

- 系统设置 → 迁移：导出用户、节点、规则、流量、订阅口令、落地库、公告、文档、Logo、设置
- 导入要打「覆盖本机数据」，确认后覆盖本机现有库并重启面板，不做两套数据合并
- 网站打不开时可用 `nft-server export` / `nft-server import`，不依赖网页

### 升级注意

- 这是**面板**改动，升面板即可，节点不用动
- 搬家包按机密文件保存；节点 token 不变，域名没换会自己连回来
- 换了面板地址：新面板改「面板地址」，节点改 `--connect` 或按新地址重装 agent
...
- 系统设置 → 面板增加「点阵」：硬边、无模糊阴影，陶土红还是唯一强调色
- 登录页和所有用户一起换；顶栏深色/浅色仍是个人的
- 正文、Logo、二维码不像素化

### 升级注意

- 这是**面板**改动，升面板即可，节点不用动
- 不选则继续用宣纸或瓷白；规则、设置、流量都不变
...
订阅页把「重置订阅链接」收到标题旁，不再单独占一块。

### 改进

- 「重置订阅链接」移到「我的订阅」标题右边，文字链接，点了仍要确认
- Clash Verge 卡片补了一行怎么用，不再大片空白
- Mihomo 去掉重复的「复制拉取地址」，只留订阅地址复制和下载 YAML
...
国内节点推送升级不再卡在「已确认接收」但进程还是旧版。用户端能自己重置订阅、停用规则，并看清入口为什么连不上。

### 新增

- 我的订阅可重置订阅链接，旧地址立刻失效
- 规则可停用/启用，停用后入口不再转发，配置还在
- 节点清单标明连不上的原因：账号禁用、过期、流量用尽、入口离线、落地到期或落地流量用尽
- 流量用到 80% / 100% 时出现和落地倒计时同款的横幅

### 修复

- 升级包改从节点实际连上的面板地址下载，不再误用海外 IP
- 下载失败或连接断开记失败，不再当成「已确认接收」
- 国内链路支持断点续传和重试；换二进制后进程会退出，由 systemd 拉起正确的 nft-daemon

### 升级注意

- 先升**面板**，再对卡住的节点点一次「推送升级」
- 仍不行就在节点上执行 `nft-upgrade`（走面板安装脚本，不经过 GitHub）
- 规则、设置、流量都不变
...
- 系统设置 → 面板增加「面板外观」：宣纸（默认）/ 瓷白
- 瓷白是石灰底 + 真白卡面，陶土红描边语言不变
- 登录页和所有用户一起换；顶栏深色/浅色仍是个人的

### 升级注意

- 这是**面板**改动，升面板即可，节点不用动
- 不选则继续用宣纸；规则、设置、流量都不变
...
- 用户、落地、仓库、公告的日期控件都能选时:分，也可直接输入 `2026-08-22 18:30`
- 只填日期时仍按当天 23:59:59（上海时间），和以前一样用满当天
- 列表里非整点到期会显示时分；23:59 仍只显示日期

### 升级注意

- 这是**面板**改动，升面板即可，节点不用动
- 已有只选日期的到期不会提前；规则、设置、流量都不变
...
- 系统设置 → 面板增加「监控地址」，填 Komari 等独立监控的 http/https 地址
- 保存后管理员侧栏「监控」下出现「主机监控」，新标签打开
- 留空则不显示；普通用户和公开页面拿不到这个地址

### 升级注意

- 这是**面板**改动，升面板即可，节点不用动
- 监控程序仍需自己安装，面板只存链接，不嵌入、不改转发
- 地址必须是浏览器能打开的公网或域名，不要填服务器本机 `127.0.0.1`
...
- 复制 / 二维码 / 订阅改名时同时改 `profile=` 和 `#`，客户端不再显示落地仓库名
- 例如由 `pv2-1062` 改为 `水宝宝-1-9月6日`
- 其它协议仍只改备注名，行为不变

### 升级注意

- 这是**面板**改动，升面板即可，节点不用动
- 升完后刷新管理端，再复制一次或让用户重新拉订阅
