...
REPO="cheesydui-cloud/kids"
RELEASE="${NFTF_RELEASE:-latest}"
INSTALL_DIR="/usr/local/sbin"
SYSTEMD_DIR="/etc/systemd/system"
ETC_DIR="/etc/nft"
SCRIPT_PATH="$INSTALL_DIR/nft-upgrade"
...
[Unit]
Description=nft web panel
Requires=nft-daemon.service
After=nft-daemon.service
...
remove_legacy_units() {
  # Earlier releases installed three separate units (nft.service for
  ...
  for unit in nft.service nft-server.service nft-agent.service; do
  ...
  # The single-binary era shipped one `nft` that every unit invoked
...
detect_existing_roles() {
  local roles=()
  if [[ -f "$SYSTEMD_DIR/nft-server.service" ]]; then
    roles+=(server)
  fi
  if [[ -f "$SYSTEMD_DIR/nft-daemon.service" ]] \
     && grep -q -- '--connect' "$SYSTEMD_DIR/nft-daemon.service"; then
    roles+=(agent)
  fi
...
do_uninstall() {
  ...
  case "$target" in
    server)
      systemctl disable --now nft-server.service 2>/dev/null || true
      rm -f "$SYSTEMD_DIR/nft-server.service"
      ...
      if [[ "$purge" -eq 1 ]]; then
        ...
        curl -sf --unix-socket /var/run/nft.sock \
             -X POST -H 'Content-Type: application/json' \
             http://daemon/v1/ruleset/panel \
             -d '{"rules":[]}' >/dev/null 2>&1 \
          || echo "警告: 未能通过 daemon API 清 panel 段（daemon 可能已停）" >&2
        rm -f /var/lib/nft/panel.db \
              /var/lib/nft/panel.db-wal \
              /var/lib/nft/panel.db-shm
    ...
    all)
      ...
      systemctl disable --now nft-daemon.service 2>/dev/null || true
      rm -f "$SYSTEMD_DIR/nft-server.service" \
            "$SYSTEMD_DIR/nft-daemon.service"
      rm -f "$INSTALL_DIR/nft-server" "$INSTALL_DIR/nft-agent" \
            "$INSTALL_DIR/nft-upgrade"
      systemctl daemon-reload
      nft delete table ip nft_forward 2>/dev/null || true
      rm -f /etc/sysctl.d/99-nft.conf
      rm -rf /etc/nft/
      # 保留 panel.db，清其他 state
      rm -f /var/lib/nft/state.json
      if getent group nft >/dev/null; then
        groupdel nft 2>/dev/null || true
      fi
      ok "已卸载所有角色（panel.db 保留在 /var/lib/nft/）"
      ;;
    *) die "未知卸载目标: $target" ;;
  esac
}
...
rollback_update() {
  ...
  systemctl restart nft-daemon.service 2>/dev/null || true
  if [[ -f "$SYSTEMD_DIR/nft-server.service" ]]; then
    systemctl restart nft-server.service 2>/dev/null || true
  fi
  exit 1
}
...
do_update() {
  ...
  [[ -f "$SYSTEMD_DIR/nft-daemon.service" ]] \
    || die "未安装：nft-daemon.service 不存在；请先 install.sh tui/server/agent"
  ...
  local want_server=0
  [[ -f "$SYSTEMD_DIR/nft-server.service" ]] && want_server=1
  ...
  for i in $(seq 1 10); do
    if systemctl is-active --quiet nft-daemon.service \
       && curl -sf --unix-socket /var/run/nft.sock \
  ...
  echo "建议查看启动日志: journalctl -u nft-daemon.service --since '1 minute ago'"
}
...
do_reset_password() {
  ...
  if [[ -f "$SYSTEMD_DIR/nft-server.service" ]]; then
    had_server=1
    note "停止 nft-server.service ..."
    systemctl stop nft-server.service 2>/dev/null || true
  fi

  note "重置 admin 密码 ..."
  if ! "$INSTALL_DIR/nft-server" --reset-admin-password "$pw" --db "$db"; then
    if [[ "$had_server" -eq 1 ]]; then
      systemctl start nft-server.service 2>/dev/null || true
    fi
    die "重置失败（admin 账号不存在？见上方错误）"
  fi

  if [[ "$had_server" -eq 1 ]]; then
    note "重启 nft-server.service ..."
    systemctl start nft-server.service
  fi
...
case "$mode" in
  ...
  server)
    if [[ -z "$addr" && -z "${PANEL_ADDR:-}" ]]; then
      ...
      if [[ -f "$SYSTEMD_DIR/nft-server.service" ]]; then
        addr="$(sed -n 's/^ExecStart=.*nft-server --addr //p' "$SYSTEMD_DIR/nft-server.service" | head -1)"
      fi
      if [[ -z "$addr" && -t 0 ]]; then
...
case "$mode" in
  tui)
    ...
    write_daemon_unit ""
    systemctl daemon-reload
    systemctl enable --now nft-daemon.service
    cat <<EOF
...
EOF
  ...
  server)
    ...
    sleep 2
    _admin_pw="$(journalctl -u nft-server.service --since '30 seconds ago' --no-pager -o cat 2>/dev/null \
    ...
    echo "daemon unit: nft-daemon.service (nft-agent daemon)"
    echo "server unit: nft-server.service (nft-server)"
    if [[ -n "$_admin_pw" ]]; then
      echo ""
      echo "管理员账号:  admin"
  ...
  agent)
    ...
    write_daemon_unit " --connect $panel_url --panel-token-file /etc/nft/panel.token${range_arg}${relay_arg}${connect_insecure_arg}"
    systemctl daemon-reload
